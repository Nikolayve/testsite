...
class ccommentComponentJoomgalleryPlugin extends ccommentComponentPlugin
{
	...
	{
		$config = ccommentConfig::getConfig('com_joomgallery');
		$row = $this->row;
	...
	public function getLink($contentId, $commentId = 0, $xhtml = true)
	{
		...
		$itemId = '&Itemid='.ccommentHelperUtils::getItemid('com_joomgallery');

		$url = JRoute::_('index.php?option=com_joomgallery&view=detail&id='.$contentId . $itemId, $xhtml).$add;
	...
	public function getAuthorId($contentId)
	{
		...
		$query->select('owner')->from('#__joomgallery')
	...
	public function getItemTitles($ids)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('id,imgtitle as title')->from('#__joomgallery')
