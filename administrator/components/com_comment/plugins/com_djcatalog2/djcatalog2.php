...
/**
 * Class ccommentComponentDJCatalog2Plugin
 *
 * @since  5.0.5
 */
class CcommentComponentDJCatalog2Plugin extends ccommentComponentPlugin
{
	/**
	...
	{
		$config = ccommentConfig::getConfig('com_djcatalog2');
		$row = $this->row;
	...
	public function getLink($contentId, $commentId = 0, $xhtml = true)
	{
		...
		$url = JRoute::_(
			'index.php?option=com_djcatalog2&view=item' .
			'&id=' . $contentId . ':' . JFilterOutput::stringURLSafe($name) .
			'&cid=' . $catid . ':' . JFilterOutput::stringURLSafe($catname) .
			'&Itemid=' . ccommentHelperUtils::getItemid('com_djcatalog2') .
			$add, $xhtml
		);
	...
	{
		$db = JFactory::getDBO();
		$query = 'SELECT c.id, c.name as cat_name FROM ' . $db->qn('#__djc2_categories') . ' AS c'
			. ' LEFT JOIN ' . $db->qn('#__djc2_items_categories') . ' AS n'
			. ' ON n.category_id = c.id'
			. ' WHERE n.item_id = ' . $db->Quote($id);
	...
	public function getAuthorId($contentId)
	{
		...
		$query->select('created_by')->from('#__djc2_items')
	...
	public function getItemTitles($ids)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('id,name as title')->from('#__djc2_items')
